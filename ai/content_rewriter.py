...
ЗАДАЧА: Полностью переписать этот пост в стиле @thedegeton, сохранив КОНКРЕТНЫЕ детали из оригинала!

КРИТИЧЕСКИ ВАЖНО:
- Сохраняй ВСЕ конкретные факты из оригинала: названия проектов, цифры, даты, курсы, суммы
- Если в посте упоминается проект "Capybobo" - упомяни его, не пиши об "игровой индустрии" абстрактно
- Если есть конкретные числа (160,904 CAPY) - сохрани их или укажи, что было указано конкретное число
- Если есть даты (5 ноября, 10 ноября) - сохрани их
- НЕ заменяй конкретику общими фразами типа "игровая индустрия", "новые проекты"
- Анализируй ЧТО КОНКРЕТНО написано в оригинале и переписывай ЭТО, а не общую тему
...
📝 СОДЕРЖАНИЕ:
- Сохраняй ВСЕ конкретные факты из оригинала: названия проектов, цифры, даты, события
- НЕ заменяй конкретику общими фразами (если про "Capybobo" - пиши про Capybobo, не про "игровую индустрию")
- Только ключевые факты из исходного поста
- Без лишних комментариев
- Четкая и понятная информация с сохранением конкретики
...
- Если в оригинале про проект "Capybobo" - пиши про Capybobo, не об "игровой индустрии"
- Если есть конкретные цифры, даты, названия - ОБЯЗАТЕЛЬНО сохрани их
- ПОЛНОСТЬЮ ПЕРЕПИШИ в стиле @thedegeton, но с сохранением конкретики
- Количество абзацев — адаптивно по длине исходного поста
- Каждый абзац содержит 2–3 предложения
- ЗАГОЛОВОК: создавай УНИКАЛЬНЫЙ по смыслу конкретного поста (1-3 слова), НЕ используй шаблоны типа "Тенденции криптовалюты"!
- ЗАГОЛОВОК должен отражать конкретную тему поста (например "Capybobo дроп" вместо "Тенденции криптовалюты")
...
- ДЕЛАЙ посты ЧЕТКИМИ и ИНФОРМАТИВНЫМИ с КОНКРЕТНЫМИ фактами из оригинала
...
КРИТИЧЕСКИ ВАЖНО:
- Сохраняй ВСЕ конкретные факты из оригинала: названия проектов, цифры, даты, курсы, суммы
- Если в посте упоминается конкретный проект (например "Capybobo") - ОБЯЗАТЕЛЬНО упомяни его, не заменяй общими фразами
- Если есть конкретные числа, даты, события - сохрани их в переписанном тексте
- НЕ заменяй конкретику общими фразами типа "игровая индустрия", "новые проекты"
- Анализируй ЧТО КОНКРЕТНО написано в оригинале и переписывай ЭТО, а не общую тему
...
- ЗАГОЛОВОК: создавай УНИКАЛЬНЫЙ по смыслу конкретного поста (1-3 слова), НЕ используй шаблоны типа "Тенденции криптовалюты"!
- ЗАГОЛОВОК должен отражать конкретную тему поста (например "Capybobo дроп" вместо "Тенденции криптовалюты")
- Сохраняй ВСЕ конкретные факты: названия проектов, цифры, даты из оригинала
- НЕ заменяй конкретику общими фразами (если про "Capybobo" - пиши про Capybobo)
- Только ключевая информация с сохранением конкретных фактов
- БЕЗ лишних комментариев
- Сохраняй основную суть исходного контента с КОНКРЕТНЫМИ деталями
- НЕ ЗАДАВАЙ вопросы аудитории в конце поста
- НЕ ИСПОЛЬЗУЙ "p.s." или дополнительные приписки
- ЧЕТКО и ИНФОРМАТИВНО с сохранением конкретики из оригинала

Всегда сохраняй конкретные факты и детали исходного контента, переписывая их в узнаваемом стиле автора."""
